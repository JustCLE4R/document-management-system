    ...
    <form action="{{ route('superadmin.setting.update') }}" method="POST">
      ...
      <div class="row justify-content-between align-items-center p-3">
        ...
        <div class="col-lg-6 col-md-6 col-sm-12 my-2">
          ...
          <input type="text" name="app_alias" class="form-control @error('app_alias') is-invalid @enderror" value="{{ old('app_alias', $settings->app_alias) }}">
          @error('app_alias') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
        ...
        <div class="col-lg-6 col-md-6 col-sm-12 my-2">
          ...
          <input type="color" name="color_primary" class="form-control @error('color_primary') is-invalid @enderror" value="{{ old('color_primary', $settings->color_primary) }}" style="height: 3rem;">
          @error('color_primary') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="col-lg-6 col-md-6 col-sm-12 my-2">
          <label class="form-label text-dark h6">Secondary Color</label>
          <input type="color" name="color_secondary" class="form-control @error('color_secondary') is-invalid @enderror" value="{{ old('color_secondary', $settings->color_secondary) }}" style="height: 3rem;">
          @error('color_secondary') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="col-lg-6 col-md-6 col-sm-12 my-2">
          <label class="form-label text-dark h6">Tertiary Color</label>
          <input type="color" name="color_tertiary" class="form-control @error('color_tertiary') is-invalid @enderror" value="{{ old('color_tertiary', $settings->color_tertiary) }}" style="height: 3rem;">
          @error('color_tertiary') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="col-lg-12 col-md-12 col-sm-12 d-flex justify-content-between">
          <button class="btn btn-success mx-1 wow fadeInRight" type="submit"><i class="bi bi-check-lg"></i> Save Settings</button>
        </div>
      </div>
    </form>
  </div>
