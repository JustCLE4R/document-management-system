...
<section class="section-padding" style="margin-top: 9vh ;">
  ...
  <div class="container border rounded shadow" style="width:70%;">
    ...
    <form action="/superadmin/department/{{ $department->id }}" id="form" method="POST" enctype="multipart/form-data">
      <div class="row justify-content-between align-items-center p-3">
        @csrf
        @method('PUT')
        <div class="col-lg-6 col-md-6 col-sm-12 my-2">
            <label for="name" class=" text-dark h6">Nama Program Studi</label>
            <input  class="form-control @error('name') is-invalid @enderror" type="text" name="name" id="name" value="{{ old('name', $department->name) }}" required>
            @if ($errors->has('name'))
